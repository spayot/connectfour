...
class ConnectFourBoard:
    """ConnectFourBoard is a representation of a connect4 board.
    It can mostly be summarized by an array with 3 types of values.
    ...
    def from_array(cls, array: np.ndarray):
        """create ConnectFourBoard using an array as an input, 
        instead of initializing it as an array of zeros."""
        # verify that shape is ok
        assert len(array.shape) == 2, "Only 2D arrays are allowed"
        ...
        assert set(np.unique(array)).issubset(valid_values), f"array values must be either {valid_values}"
        
        board = cls(array.shape)
        board.array = array.astype(int)
        
        return board
    
    def __getitem__(self, *args):
        """allows to retrieve rows and individual values from board position"""
    ...
    def copy(self):
        copied_array = np.copy(self.array)
...
class ConnectFourGameState: 
    ...
    def game_result(self):
        """determine whether the game state corresponds to an end state and which player won if any. 
        ...
        # apply convolutions
        convolutions = np.array([convolve(board_array, kernel, mode='constant', cval=0) for kernel in kernels])
    ...
    def is_move_legal(self, action):
        ...
        x_in_range = (0 <= action.x_coordinate < self.board_size[1])
        if not x_in_range:
            return False
