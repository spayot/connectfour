...
class ConnectFourAction:
    """Actions in ConnectFour are defined by who is the player about to
    play and which column do they insert their disc `x_coordinate`."""
...
def _assert_board_is_valid(board: np.ndarray, game_config: ConnectFourGameConfig):
    ...
    # validate shape
    assert (
        board.shape == game_config.shape
    ), f"""the board shape is {board.shape}, but the 
    game configuration defines a {game_config.shape} shape"""

    # verify that dtype is valid
    assert np.issubdtype(
        board.dtype, np.number
    ), "board array should be of dtype np.int* "

    # verify that values are valid
    valid_values = {Player.x.value, Player.o.value, 0}
    assert set(np.unique(board)).issubset(
        valid_values
    ), f"array values must be either {valid_values}"
...
class ConnectFourGameState:
    ...
    def game_result(self) -> Optional[int]:
        ...
        convolutions = np.array(
            [
                convolve(self.board, kernel, mode="constant", cval=0)
    ...
    def is_move_legal(self, action: ConnectFourAction) -> bool:
    ...
    def move(self, action: ConnectFourAction) -> ConnectFourGameState:
    ...
    def get_legal_actions(self) -> list[ConnectFourAction]:
        indices = np.where(self.board[0, :] == 0)[0]
        return [ConnectFourAction(idx, self.next_player) for idx in indices]
