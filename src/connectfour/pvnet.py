...
from .config import pvn_config, board_config
...
def conv_block(input_tensor, kernel_size, filter, l2_const):
    x = input_tensor
    x = layers.Conv2D(filter, kernel_size, padding='same', 
...
def residual_block(input_tensor, kernel_size, filter, l2_const):
    ...
    shortcut = layers.Conv2D(filter, kernel_size = (1,1), padding = 'same',
                             kernel_regularizer=l2(l2_const),
...
class PolicyValueNet():
    ...
    def build_model(self) -> None:
        ...
        for i in range(pvn_config['block_size']):
        ...
        value_net = layers.Dense(256,
            activation='relu',
            kernel_regularizer=l2(self.l2_const),)(value_net)
        self.value_net = layers.Dense(1, 
            activation="tanh", 
            kernel_regularizer=l2(self.l2_const),
            name='value_head')(value_net)
    ...
    def get_train_fn(self):
        ...
        def train_fn(input_boards, policy, value):
            history = self.model.fit(np.asarray(input_boards), 
    ...
    def evaluate_state(self, state: ConnectFourGameState) -> (np.ndarray, float):
